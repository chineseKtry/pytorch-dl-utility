...
import subprocess
import shutil
from os.path import join, dirname, basename, exists, realpath, splitext
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='PyTorch + Hyperband for genomics')
    ...
    parser.add_argument('-hi', '--hyperiter', dest='hyperiter', default=20,
                        type=int, help='Num of max iteration for each hyper-param config')
    parser.add_argument('-te', '--trainepoch', default=20,
    ...
    parser.add_argument('-bs', '--batchsize', default=100,
                        type=int, help='Batchsize in SGD-based training')
...
def train_func(model, weightfile2save):
    ...
    if args.datamode == 'generator':
        ...
        history_callback = model.fit_generator(
            hb.BatchGenerator(args.batchsize, join(
...
def load_model(weightfile2load=None):
    ...
    model.compile(loss=best_lossfunc, optimizer=best_optim.from_config(
        best_optim_config), metrics=['categorical_accuracy'])
    return model
...
if __name__ == '__main__':
    ...
    hb = Hyperband(model_def.get_params, model_def.try_params,
                   args.topdir, max_iter=args.hyperiter, datamode=args.datamode)
    ...
    if args.hyper:
        ...
        pprint('best parameters', best_result['params'])
